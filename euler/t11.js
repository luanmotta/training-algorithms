...
function solution() {
  ...
  // left to right
  for (let i = 0; i < GRID; i++) {
    for (let j = 0; j <= GRID - RANGE; j++) {
      process(m[i][j], m[i][j + 1], m[i][j + 2], m[i][j + 3])
    }
  }


  // up to down
  for (let i = 0; i <= GRID - RANGE; i++) {
    for (let j = 0; j < GRID; j++) {
      process(m[i][j], m[i + 1][j], m[i + 2][j], m[i + 3][j])
    }
  }
  ...
  for (let i = 0; i <= GRID - RANGE; i++) {
    for (let j = 0; j <= GRID - RANGE; j++) {
      process(m[i][j], m[i + 1][j + 1], m[i + 2][j + 2], m[i + 3][j + 3])
    }
  }

  // diagonal right up to left down
  for (let i = 0; i <= GRID - RANGE; i++) {
    for (let j = GRID; j >= RANGE; j--) {
      process(m[i][j], m[i + 1][j - 1], m[i + 2][j - 2], m[i + 3][j - 3])
    }
  }
