...
function solution(DNA, start, end) {
  const dictionary = {
    'A': '1',
    'C': '2',
    'G': '3',
  ...
  const result = []
  const DNAnumbers = DNA.split('').map(n => dictionary[n]).join('')
  for (let i = 0, min = 0; i < start.length; i++) {
    const sliced = DNAnumbers.substring(start[i], end[i] + 1)
    sliced.split('').map((n, index) => {
